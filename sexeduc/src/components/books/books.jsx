...
export function Books({ isOpen }) {
	...
	const texte = () => {
		if (contenu === 'menu') return (<>
			<p onClick={() => setContenu('sciences')}>Science Atomique</p>
			<p onClick={() => setContenu('bricolage')}>Bricolage Express</p>
			<p onClick={() => setContenu('orange')}>Jus d'orange Frais</p>
			<p onClick={() => setContenu('vert')}>Jardinage Vert</p>
			<p onClick={() => setContenu('banane')}>Cuisine à la Banane</p>
			<p onClick={() => setContenu('robotique')}>Robotique Explorée</p>
		</>
		)

		if (contenu === 'sciences') return (
			<>
				<div className="back-book" onClick={() => setContenu('menu')}>Retour ⏎</div>
				<div className="book">
		...
		if (contenu === 'orange') return (
			<>
				...
				<img src="./ressources/level1/books/oranges.jpg" />
			</>
		)
		if (contenu === 'vert') return (
			<>
				...
				<img src="./ressources/level1/books/plantes.jpg" />
			</>
		)
		if (contenu === 'robotique') return (
			<>
				...
				<img src="./ressources/level1/books/robot.jpg" />
			</>
		)
	}
